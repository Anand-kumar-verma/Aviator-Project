...
const Login = () => {
  ...
  const loginFunction = async(reqbody)=>{
    try{
      const response = await axios.post('https://gameszone.life/api/aviator/login',reqbody)
    ...
    <div>
      ...
      <form className="pt-5">
        ...
        onClick={()=>fk.handleSubmit()}
         className="!w-full !mt-4" variant="contained" disableElevation>
          Login
        </Button>
      ...
      >
        <MenuItem onClick={()=>handleClose(91)} value="91">India +91</MenuItem>
        <MenuItem onClick={()=>handleClose(880)} value="880">Bangladesh +880</MenuItem>
      </Menu>
    </div>
  );
};
