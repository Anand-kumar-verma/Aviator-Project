...
const SpentBetLeft = ({ milliseconds, seconds, fk, formik }) => {
  ...
  const pre_amount =
    client.getQueriesData("walletamount")?.[0]?.[1]?.data?.wallet || 0;
  ...
  const getHistory = async () => {
    ...
    try {
      const response = await axios.get(
        `https://gameszone.life/api/aviator/bet_histroy?userid=${userid}&limit=${10}`
      );
  ...
  const cashOut = async (sec, mili) => {
    ...
    try {
      const response = await axios.get(
        `https://gameszone.life/api/aviator/cash_out?userid=${reqbody.userid}&amount=${reqbody.amount}&multiplier=${reqbody.multiplier}&gamesno=${reqbody?.gameno}`
      );
